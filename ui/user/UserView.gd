...
onready var _role: LineEdit = get_node(role_path) if role_path else null
onready var _may_borrow: CheckBox = get_node(may_borrow_path) if may_borrow_path else null
...
func set_user(u: Dictionary):
    if is_inside_tree():
        ...
        if _role: _role.text = u.get("role", "-")
...
func get_user() -> Dictionary:
    if is_inside_tree():
        return {
            ...
            role = _role.text if _role else  "-",
            may_borrow = not _may_borrow or _may_borrow.pressed,
        }
    return {}
